  ...
  return (
    <>
      ...
      <div key={review._id} className='review'>
        ...
        <div id='left-side' style={{ backgroundImage: `url('${profile === 'self' || profile === 'self-profile' ? user?.image : review.user.image}')` }}>

          <div className='general-user-image' />
        </div>
        <div id='right-side'>
          <div className='review-title'>
            <div>
              <h5>{review.title}</h5>
              <h6>{profile === 'self' ? user?.name : review.user.name}</h6>
            </div>
